    ...
    try {
      ...
      const session = await ai.live.connect({
        ...
        config: {
          ...
          tools: [
            { functionDeclarations: [openWebsiteTool, searchTool, getDateTimeTool, playMusicTool, sendWhatsAppMessageTool, initiateCallTool] }
          ],
...
You MUST politely speak to the user to inform them: "Pranav, ${data.name} ka message aaya tha. Maine unhe auto-reply bhej diya hai." Do NOT ask for permission.`;
    ...
    // Connect to Telegram Bot SSE
    const tgEventSource = new EventSource('http://localhost:4002/stream');
      
    tgEventSource.onmessage = async (event) => {
      const data = JSON.parse(event.data);
      if (data.name && data.message) {
        console.log(`[TG SSE] Incoming from ${data.name}: ${data.message}`);
        
        if (sessionRef.current) {
            const systemPrompt = `[SYSTEM NOTIFICATION] An incoming TELEGRAM MESSAGE arrived from ${data.name}: "${data.message}". 
IMPORTANT: An auto-reply was ALREADY SENT on your behalf.
You MUST politely speak to the user to inform them: "Pranav, Telegram par ${data.name} ka message aaya tha. Maine unhe auto-reply bhej diya hai." Do NOT ask for permission.`;
